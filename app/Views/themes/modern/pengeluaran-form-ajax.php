...
<form method="post" action="" class="form-horizontal form-pendapatan-lain px-3" enctype="multipart/form-data">
	<div>
		...
		<div class="form-group row mb-2">
			...
			<div class="col-sm-9">
				<div id="list-file-bukti" class="d-flex flex-wrap">
					<?php
					if (!empty($pengeluaran['file'])) {
						...
						foreach ($pengeluaran['file'] as $val) {
							...
							echo '<div class="input-group mb-2 me-2 flex-nowrap" style="width:auto">
									<span class="input-group-text rounded">
										<img class="jwd-img-thumbnail file-thumbnail" src="' . base_url() . '/public/images/filepicker_images/' . $icon_file . '.png" style="max-height: 22px;">
										<span class="ms-3">' . $val['nama_file'] . '</span>
									</span>
									<input type="hidden" id="id-file-picker-' . $val['id_file_picker'] . '" name="id_file_picker[]" value="' . $val['id_file_picker'] . '">
									<button type="button" class="btn btn-danger btn-xs btn-del-bukti"><i class="fas fa-times"></i></button>
								</div>';
						}
					}
