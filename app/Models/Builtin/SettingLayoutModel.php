...
class SettingLayoutModel extends \App\Models\BaseModel
{
	...
	public function getPegawaiSetting() {
		$sql = 'SELECT * FROM setting_pegawai WHERE id_pegawai = ? AND type="layout"';
		$data = $this->db->query($sql, $_SESSION['pegawai']['id_pegawai'])
					->getRowArray();
		return $data;
	}
	...
	public function saveData() 
	{
		...
		if (key_exists('update_all', $_SESSION['pegawai']['permission']))
		{
			$this->db->transStart();
		...
		} else if (key_exists('update_own', $_SESSION['pegawai']['permission'])) 
		{
			$this->db->transStart();
			$this->db->table('setting_pegawai')->delete(['id_pegawai' => $_SESSION['pegawai']['id_pegawai'], 'type' => 'layout' ]);
			$result = $this->db->table('setting_pegawai')->insert([
