...
class RecoveryModel extends \App\Models\BaseModel
{
	...
	public function sendLink() 
	{
		...
		$sql = 'SELECT * FROM pegawai WHERE email = ?';
		$pegawai = $this->db->query($sql, $_POST['email'])->getRowArray();
		
		$this->db->transStart();
		
		$this->db->table('pegawai_token')->delete(['action' => 'recovery', 'id_pegawai' => $pegawai['id_pegawai']]);
		$auth = new Auth;
	...
	public function updatePassword($dbtoken) {
		$this->db->table('pegawai_token')->delete(['selector' => $dbtoken['selector']]);
		$update = $this->db->table('pegawai')->update(['password' => password_hash($_POST['password'], PASSWORD_DEFAULT)], ['id_pegawai' => $dbtoken['id_pegawai']]);
		return $update;
	}
}
?>