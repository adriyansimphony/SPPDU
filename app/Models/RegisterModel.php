...
class RegisterModel extends \App\Models\BaseModel
{
	...
	public function resendLink() 
	{
		...
		$pegawai = $this->getPegawaiByEmail( $_POST['email'] );
					
		$this->db->transBegin();
		
		$this->db->table('pegawai_token')->delete(['action' => 'activation', 'id_pegawai' => $pegawai['id_pegawai']]);
		...
		$data_db['id_pegawai'] = $pegawai['id_pegawai'];
		$data_db['created'] = date('Y-m-d H:i:s');
		$data_db['expires'] = date('Y-m-d H:i:s', strtotime('+1 hour'));
		
		$insert_token = $this->db->table('pegawai_token')->insert($data_db);
		
		// $save = true;
		if ($insert_token)
		{
			$send_email = $this->sendConfirmEmail($token, $pegawai, 'link_aktivasi');
	...
	public function checkPegawaiById($id_pegawai) 
	{
	...
	public function updatePegawai($dbtoken) {
		$this->db->transStart();

		$query = $this->db->table('pegawai_token')->delete(['selector' => $dbtoken['selector']]);
		$query = $this->db->table('pegawai_token')->delete(['action' => 'register', 'id_pegawai' => $dbtoken['id_pegawai']]);
		
		$query = $this->db->table('pegawai')->update(['verified' => 1], ['id_pegawai' => $dbtoken['id_pegawai']]);
	...
	public function insertPegawai() 
	{
		...
		$data_db['nip_pegawai'] = $_POST['nip_pegawai'];
		...
		$insert_pegawai = $this->db->table('pegawai')->insert($data_db);
		...
		if (!$id_pegawai)
		{
			...
			$data_db = [];
			$data_db['id_pegawai'] = $id_pegawai;
			$data_db['id_role'] = $id_role;
			$insert_pegawai = $this->db->table('pegawai_role')->insert($data_db);
	...
	private function sendConfirmEmail($token, $pegawai, $type = 'email_confirm')
	{
