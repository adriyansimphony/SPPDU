...
class PendapatanLainModel extends \App\Models\BaseModel
{
	...
	public function getAllPendapatanJenis() {
	...
	public function getJmlDataPendapatanLain() {
		$sql ='SELECT COUNT(*) AS jml FROM pendapatan WHERE id_pembayar IS NOT NULL';
		$result = $this->db->query($sql)->getRowArray();
		return $result['jml'];
	}
	
	public function getPendapatanLainById() {
	...
	public function deleteAllData() {
		...
		try {
			...
			foreach ($list_table as $table) 
			{
				if ($table == 'pendapatan') 
				{
					$sql = 'SELECT * FROM pendapatan WHERE id_pembayar IS NOT NULL';
					$result = $this->db->query($sql)->getResultArray();
					
					if ($result) {
						foreach ($result as $val) {
							$list_id[] = $val['id_pendapatan'];
						}
						
						$this->db->table($table)->delete(['id_pembayar IS NOT NULL ' => null]);
						$sql = 'SELECT MAX(id_pendapatan) as max FROM pendapatan';
						$result = $this->db->query($sql)->getRowArray();
						$auto_increment = $result ? $result['max'] + 1 : 1;
						$sql = 'ALTER TABLE ' . $table . ' AUTO_INCREMENT ' . ($auto_increment);
					}
				} else {
					if ($list_id) {
						$sql = 'DELETE FROM ' . $table . ' WHERE id_pendapatan IN(' . join (',', $list_id) . ')';
						$this->db->query($sql);
					}
				}
			}
	...
	public function saveData() {
		...
		$data_db['id_pembayar'] = $_POST['id_pembayar'];
		$data_db['using_invoice'] = $_POST['using_invoice'];
		// Invoice
		...
		$data_db['total_pembayaran'] = $total;
	...
	public function countAllDataPembayaranJenis() {
	...
	public function getListDataPembayaranJenis() {

		$columns = $this->request->getPost('columns');
		$where = ' WHERE id_pendapatan_sumber != "1" ';
	...
	public function countAllData() {
		...
		$sql = 'SELECT COUNT(*) AS jml 
				FROM pendapatan WHERE id_pembayar IS NOT NULL';
		$result = $this->db->query($sql)->getRow();
		return $result->jml;
	}
	...
	public function getListData() {
		...
		$where = ' WHERE id_pembayar IS NOT NULL ';
		if (!empty($_GET['kelas'])) {
			$where .= ' AND group_kelas = ' . $_GET['kelas'];  
		}
		// Search
		...
		$start = $this->request->getPost('start') ?: 0;
		$length = $this->request->getPost('length') ?: 10;
		$sql = 'SELECT *, SUM(nilai_bayar) AS nilai_bayar, GROUP_CONCAT(nama_pendapatan_jenis) AS nama_pendapatan_jenis
				FROM pendapatan
				LEFT JOIN pembayar USING(id_pembayar)
				LEFT JOIN pendapatan_detail USING(id_pendapatan)
				LEFT JOIN pendapatan_jenis USING(id_pendapatan_jenis)
	...
	public function countAllDataPembayar() {
	...
	public function getListDataPembayar() {
