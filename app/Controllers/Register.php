...
class Register extends \App\Controllers\BaseController
{
	...
	public function index()
	{
		...
		if (!empty($_POST['submit'])) 
		{
			...
			// Submit data
			if (!$error) {		
				$message = $this->model->insertPegawai();
	...
	public function confirm() 
	{
		...
		if (!$error) {
			...
			{
				$error = false;
				$pegawai = $this->model->checkPegawaiById($dbtoken['id_pegawai']);
				$auth = new Auth;
				
				if ($pegawai['verified'] == 1) {
		...
		if (!$error)
		{
			$update = $this->model->updatePegawai($dbtoken);
	...
	private function validateForm() 
	{
		...
		// Cek email
		$pegawai_exists = $this->model->getPegawaiByEmail($_POST['email']);
		if ($pegawai_exists) {
			if($pegawai_exists['verified'] == 0) {
		...
		$validation->setRules(
			[
				...
				'email' => [
					'label'  => 'Email',
					'rules'  => 'trim|required|valid_email|is_unique[pegawai.email]',
					'errors' => [
	...
	private function validateFormResendlink() {
		...
		if ($validate) 
		{		
			$pegawai = $this->model->getPegawaiByEmail($_POST['email']);
			if ($pegawai) {
				if ($pegawai['verified'] == 1) {
