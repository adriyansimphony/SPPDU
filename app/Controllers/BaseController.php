...
class BaseController extends Controller
{
	...
	private $methodName;
	// protected $actionPegawai;
	protected $moduleURL;
	// protected $moduleRole;
	protected $modulePermission;
	protected $pegawaiPermission;
	...
	-- Jika pegawai telah login maka method $this->mustNotLoggedIn(); akan mengarahkan pegawai ke halaman default yang di assign pada pegawai tersebut
	...
	*/
	public function __construct() 
	{
		...
		$this->data['site_title'] = 'Aplikasi Siswa dan Pembayaran SPP Sekolah';
		$this->data['site_desc'] = 'Aplikasi Siswa dan Pembayaran SPP Sekolah lengkap dengan berbagai fitur untuk memudahkan pengembangan aplikasi';
		...
		if ($this->isLoggedIn) {
			$pegawai_setting = $this->model->getPegawaiSetting();
			
			if ($pegawai_setting) {
				$this->data['app_layout'] = json_decode($pegawai_setting->param, true);
			}
		...
		if ($this->isLoggedIn) 
		{
			$this->data['pegawai'] = $this->pegawai;

			// List action assigned to role
			$this->data['action_pegawai'] = $this->pegawaiPermission;
			...
			$this->data['module_role'] = $this->model->getDefaultPegawaiModule();
						
			$this->getModulePermission();
			$this->getListPermission();
			
			$result = $this->model->getAllModulePermission($_SESSION['pegawai']['id_pegawai']);
			...
			$_SESSION['pegawai']['all_permission'] = $all_module_permission;
	...
	{ 
		$pegawai_role = $this->session->get('pegawai')['role'];
				
		if ($this->isLoggedIn && $this->currentModule['nama_module'] != 'login') 
		{
			$current_pegawai = $this->model->getPegawaiById($this->pegawai['id_pegawai']);
			if ($current_pegawai['status'] != 'active') {
				$this->data['content'] = 'Status akun Anda ' . ucfirst($current_pegawai['status']);
				$this->exitError($this->data);
			}
			if (!$pegawai_role) {
				$this->printError('Pegawai belum memiliki role');
				exit;
			}
			...
			if ($this->modulePermission) 
			{
				...
				if ($this->currentModule['nama_module'] != 'login' ) {
					...
					$role_exists = false;
					foreach ($pegawai_role as $id_role => $val) {
						if (key_exists($id_role, $this->modulePermission)) {
							$this->pegawaiPermission = $this->modulePermission[$id_role];
							unset($this->pegawaiPermission['null']);
							$role_exists = true;
							break;
						}
					}
					
					if ($this->pegawaiPermission) {
						$session_pegawai = $this->session->get('pegawai');
						$session_pegawai['permission'] = $this->pegawaiPermission;
						$this->session->set('pegawai', $session_pegawai);
					}
	...
	protected function redirectOnLoggedIn() {
		if ($this->isLoggedIn) {
			...
			header('Location: ' . $this->config->baseURL . $this->pegawai['default_module']['nama_module']);
			// header('Location: ' . $this->config->baseURL . $this->pegawai['default_module']['nama_module']);
			// redirect($this->router->default_controller);
		}
	}
	
	/* Redirect Pegawai setelah login */
	protected function mustNotLoggedIn() {
		if ($this->isLoggedIn) {	
			if ($this->currentModule['nama_module'] == 'login') {
				
				$redirect_url = '';
				if ($this->pegawai['default_page_type'] == 'url') {
					$redirect_url = str_replace('{{BASE_URL}}', $this->config->baseURL, $this->pegawai['default_page_url']);
				} else if ($this->pegawai['default_page_type'] == 'id_module') {
					$redirect_url = $this->config->baseURL . $this->pegawai['default_module']['nama_module'];
				} else {
					$redirect_url = $this->config->baseURL . $this->pegawai['role'][$this->pegawai['default_page_id_role']]['nama_module'];
				}
		...
		if ($this->config->checkRoleAction['enable_global']) 
		{
			...
			if ($method == 'add' || $method=='edit') 
			{
				if (key_exists($method, $list_action)) {
					...
					foreach ($this->pegawaiPermission as $val) 
					{
						$exp = explode('_', $val);
						$exists = false;
	...
	protected function pegawaiCan($action) {
		if (!$this->pegawaiPermission) {
			return '';
		}
	...
	protected function mustHavePermission($permission) {
		...
		if (!in_array($permission, $this->pegawaiPermission)) {
	...
	{
		if (!in_array($action, $this->pegawaiPermission)) {
			if ($exit) {
		...
		return in_array($action, $this->pegawaiPermission);
	}
	...
	protected function hasPermissionPrefix($action, $return = false) {
		...
		{
			$exp = explode('_', $val);
			$pegawai_action = trim($exp[0]);
			if ($pegawai_action == $action || $pegawai_action == $action . '_all') {
				$has_permission = true;
				break;
			}
		}
	...
	{	
		...
		if ($this->actionPegawai['read_data'] == 'own') {
			return ' WHERE ' . $column . ' = ' . $_SESSION['pegawai']['id_pegawai'];
		}
		...
		if (key_exists('read_own', $this->pegawaiPermission) && !key_exists('read_all', $this->pegawaiPermission)) {
			return ' WHERE ' . $column . ' = ' . $_SESSION['pegawai']['id_pegawai'];
		}
