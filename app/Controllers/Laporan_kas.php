...
use App\Models\LaporanKasModel;

class Laporan_kas extends \App\Controllers\BaseController
{
	public function __construct() {
		
		parent::__construct();
		
		$this->model = new LaporanKasModel;	
		$this->data['site_title'] = 'Laporan Kas';
		...
		$this->addJs ( $this->config->baseURL . 'public/themes/modern/js/laporan-kas.js');
	}
	...
	public function index()
	{
		...
		$this->view('laporan-kas.php', $this->data);
	}
	...
	public function setData() 
	{
		...
		$result['kas_awal'] = $this->model->getKasAwal($start_date_db);
		$result['penyesuaian_kas'] = $this->model->getPenyesuaianKas($start_date_db, $end_date_db);
	...
	public function printLaporan() 
	{
		$result = $this->setData();
		$this->data = array_merge($this->data, $result);
		echo view('themes/modern/laporan-kas-print.php', $this->data);
	}
	...
	public function ajaxExportExcel() {
		...
		$excel->getProperties()->setCreator('Jagowebdev.com')
			...
			->setDescription('Laporan Kas')
			->setKeywords('laporan Kas')
			->setCategory('laporan Kas');
		...
		$num_row = 5;
		// Saldo awal kas
		$sheet->setCellValue('A' . $num_row, 'Saldo Kas ' . format_tanggal($start_date_db));
		$sheet->setCellValue('C' . $num_row, $kas_awal);
		...
		if ($penyesuaian_kas) {
			$sheet->setCellValue('A' . $num_row, 'Penyesuaian Kas ');
			$sheet->getStyle('A' . $num_row . ':' . 'A' . $num_row)->getFont()->setBold(true);
			$num_row++;
			$total_penyesuaian_kas = $penyesuaian_kas['penambahan_kas'] + $penyesuaian_kas['pengurangan_kas'];
		}
		
		if (@$penyesuaian_kas['penambahan_kas']) {
			$sheet->setCellValue('A' . $num_row, 'Penambahan Kas');
			$sheet->setCellValue('C' . $num_row++, $penyesuaian_kas['penambahan_kas']);
		}
		
		if (@$penyesuaian_kas['pengurangan_kas']) {
			$sheet->setCellValue('A' . $num_row, 'Pengurangan Kas');
			$sheet->setCellValue('C' . $num_row++, $penyesuaian_kas['pengurangan_kas']);
		}
		
		if ($penyesuaian_kas) {
			$sheet->setCellValue('A' . $num_row, 'Total Penyesuaian Kas');
			$sheet->setCellValue('C' . $num_row, $total_penyesuaian_kas);
			$sheet->getStyle('A' . $num_row . ':' . 'C' . $num_row)->getFont()->setBold(true);
			$num_row++;
		}
		...
		$kas = $total_pendapatan - $total_pengeluaran;
	
		$sheet->setCellValue('A' . ++$num_row, 'Total Pengeluaran');
		$sheet->setCellValue('C' . $num_row, '-' . $total_pengeluaran);
		$sheet->getStyle('A' . $num_row . ':' . 'C' . $num_row)->getFont()->setBold(true);
		
		$sheet->setCellValue('A' . ++$num_row, 'Saldo Kas ' . format_tanggal($end_date_db));
		$sheet->setCellValue('C' . $num_row, $kas);
		$sheet->getStyle('A' . $num_row . ':' . 'C' . $num_row)->getFont()->setBold(true);
	...
	public function ajaxExportPdf() 
	{
		...
		$pdf->SetTitle('Laporan Kas ' . $_GET['daterange']);
		$pdf->SetSubject('Laporan Kas');
		...
		$pdf->Cell(0, 0, 'Laporan Kas', 0, 1, 'C', 0, '', 0, false, 'T', 'M' );
		...
		<table border="0" cellspacing="0" cellpadding="1">
			<tbody>
				<tr border="0">
					<td style="width:75%;font-weight:bold">Saldo Kas {$format_tanggal($start_date_db)}</td>
					<td style="width:8%"></td>
					<td style="width:12%" align="right">{$format_number($kas_awal)}</td>
				</tr>
		EOD;
		...
		if ($penyesuaian_kas) {
			$tbl .= <<<EOD
				<tr border="0">
					<td style="width:75%;font-weight:bold">Penyesuaian Kas</td>
		...
		if (@$penyesuaian_kas['penambahan_kas']) {
			$tbl .= <<<EOD
				<tr border="0">
					<td style="width:75%;">Penambahan Kas</td>
					<td style="width:12%"></td>
					<td style="width:13%" align="right">{$format_number($penyesuaian_kas['penambahan_kas'])}</td>
				</tr>
		EOD;
		}
		...
		if (@$penyesuaian_kas['pengurangan_kas']) {
			$tbl .= <<<EOD
				<tr border="0">
					<td style="width:75%;">Pengurangan Kas</td>
					<td style="width:12%"></td>
					<td style="width:13%" align="right">{$format_number($penyesuaian_kas['pengurangan_kas'])}</td>
				</tr>
		EOD;
		}
		...
		if ($penyesuaian_kas) {
			$tbl .= <<<EOD
				<tr border="0">
					<td style="width:75%;font-weight:bold">Total Penyesuaian Kas</td>
					<td style="width:12%"></td>
					<td style="width:13%;font-weight:bold" align="right">{$format_number($total_penyesuaian_kas)}</td>
				</tr>
		EOD;
		}
		...
		$tbl .= <<<EOD
					...
					<tr>
						<td style="width:75%;font-weight:bold">Saldo Kas {$format_tanggal($end_date_db)}</td>
						<td style="width:12%;"></td>
						<td style="width:13%;font-weight:bold" align="right">{$format_number($kas, true)}</td>
					</tr>
			EOD;
		...
		$filename = 'Laporan Kas ' . $_GET['daterange'] . '.pdf';
