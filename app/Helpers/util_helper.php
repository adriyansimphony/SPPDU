...
function format_tanggal($date, $format = 'dd mmmm yyyy', $menit = false) 
{
...
function has_permission($action, $nama_module = '') 
{
	...
	$exists = false;
	if ($nama_module) {
		foreach ($pegawai['all_permission'] as $id_permission => $list_permission) {
		...
		if (in_array($action, $pegawai['permission'])) {
			$exists = true;
		}
	}
...
function format_date($tgl, $nama_bulan = true) {
	if ($tgl == '0000-00-00 00:00:00' || !$tgl || $tgl == '0000-00-00') {
		return false;
	}
	$exp = explode (' ', $tgl);
...
function kategori_list($result)
{
	...
	foreach ($result as $key => $data)
	{
		if (!$key || empty($data['id_pengeluaran_kategori'])) // Highlight OR No parent
			continue;
		
		$thisref = &$refs[ $data['id_pengeluaran_kategori'] ];
		foreach ($data as $field => $value) {
			$thisref[$field] = $value;
		}
		...
		if ($data['id_parent'] == 0) {
			...
			$list[ $data['id_pengeluaran_kategori'] ] = &$thisref;
		} else {
			
			$thisref['depth'] = ++$refs[ $data['id_pengeluaran_kategori']]['depth'];			
			$refs[ $data['id_parent'] ]['children'][ $data['id_pengeluaran_kategori'] ] = &$thisref;
		}
	}
	set_depth($list);	
	return $list;
