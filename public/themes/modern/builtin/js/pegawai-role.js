jQuery(document).ready(function () {
	...
	$('#table-result').delegate('a[data-action="remove-role"]', 'click', function() {
		$this = $(this);
		// Module - Pegawai id
		id_pegawai = $this.attr('data-id-pegawai');
		id_role = $this.attr('data-role-id');
		$.ajax({
			type: 'POST',
			url: base_url + 'builtin/pegawai-role/delete',
			data: 'id_pegawai='+ id_pegawai +'&id_role=' + id_role,
			dataType: 'text',
	...
	$('#table-result').delegate('.btn-edit', 'click', function(e) {
				...
				success: {
					...
					callback: function() 
					{
						...
						$.ajax({
							...
							data: $checkbox_wrapper.serialize() + '&id_pegawai=' + id_pegawai,
							dataType: 'text',
							success: function (data) {
								data = $.parseJSON(data);
								console.log(data);
		...
		var id = $(this).attr('data-id-pegawai');
