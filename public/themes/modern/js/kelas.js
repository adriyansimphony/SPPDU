...
jQuery(document).ready(function () {
	...
	function showForm(type='add', id = '') {
		...
		$.get(base_url + 'kelas/ajaxGetFormData?id=' + id, function(html){
			$button.prop('disabled', false);
			$bootbox.find('.modal-body').empty().append(html);
		});
	};
	
	$('.btn-delete-all-kelas').click(function() {
		$this = $(this);
		$bootbox =  bootbox.dialog({
			title: 'Hapus Semua Data',
			message: '<div class="px-2">' +
						'<p>Tindakan ini akan menghapus semua data pada database tabel kelas</p>' +
				'</div>'+
			'</form>',
			buttons: {
				cancel: {
				...
				success: {
					...
					callback: function() 
					{
						...
						$.ajax({
							type: 'GET',
							url: base_url + 'kelas/ajaxDeleteAllKelas',
							dataType: 'text',
							success: function (data) {
								data = $.parseJSON(data);
								console.log(data);
