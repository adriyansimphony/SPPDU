...
jQuery(document).ready(function () {
	...
	function showForm(type='add', id = '') {
		...
		$bootbox.find('.modal-dialog').css('min-width', 600);
		$.get(base_url + 'penyesuaian-kas/ajaxGetFormData?id=' + id, function(html){
			$button.prop('disabled', false);
			$bootbox.find('.modal-body').empty().append(html);
			$bootbox.find('.flatpickr').flatpickr({dateFormat: 'd-m-Y'});
	...
	$('.btn-delete-all-data').click(function() {
		...
		$bootbox =  bootbox.dialog({
			...
			message: '<div class="px-2">' +
						'<p>Tindakan ini akan menghapus semua data pada database tabel penyesuaian_kas</p>' +
				'</div>'+
			'</form>',
			buttons: {
				cancel: {
				...
				success: {
					...
					callback: function() 
					{
						...
						$.ajax({
							type: 'GET',
							url: base_url + 'penyesuaian-kas/ajaxDeleteAllData',
							dataType: 'text',
							success: function (data) {
								data = $.parseJSON(data);
								console.log(data);
