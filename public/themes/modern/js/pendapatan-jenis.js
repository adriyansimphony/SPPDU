...
jQuery(document).ready(function () {
	...
	$('body').delegate('#using-periode', 'change', function(e) {
		if (this.value == 'Y') {
			$('#row-jenis-periode').show();
		} else {
			$('#row-jenis-periode').hide();
		}
	})
		...
		$bootbox = bootbox.confirm({
			...
			callback: function(confirmed) {
				if (confirmed) {
					...
					$.ajax({
						type: 'POST',
						url: base_url + 'pendapatan-jenis/ajaxDeleteData',
						data: 'id=' + id,
						dataType: 'json',
	...
	function showForm(type='add', id = '') {
			...
			buttons: {
				...
				success: {
					...
					callback: function() 
					{
						...
						$.ajax({
							type: 'POST',
							url: base_url + 'pendapatan-jenis/ajaxSaveData',
		...
		$.get(base_url + 'pendapatan-jenis/ajaxGetFormData?id=' + id, function(html){
			$button.prop('disabled', false);
	...
	$('.btn-delete-all-data').click(function() {
		...
		$bootbox =  bootbox.dialog({
			...
			message: '<div class="px-2">' +
						'<p>Tindakan ini akan menghapus semua data pada database tabel pendapatan_jenis dan pendapatan_jenis_sumber</p>' +
				'</div>'+
			'</form>',
			buttons: {
				cancel: {
				...
				success: {
					...
					callback: function() 
					{
						...
						$.ajax({
							type: 'GET',
							url: base_url + 'pendapatan-jenis/ajaxDeleteAllData',
							dataType: 'text',
							success: function (data) {
								data = $.parseJSON(data);
								console.log(data);
	...
	$('body').delegate('#id-pendapatan-sumber', 'change', function() {
		if ($.inArray('1', $(this).val()) > -1) {
			$('#row-perlu-tagihan-siswa').show();
		} else {
			$('#perlu-tagihan-siswa').val('N');
			$('#row-perlu-tagihan-siswa').hide();
		}
