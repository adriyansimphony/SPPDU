	...
	$('body').delegate('.add-file', 'click', function() {
		...
		jwdfilepicker.init({
			...
			onSelect: function ($elm) 
			{
				...
				console.log(meta);
				if ($('#id-file-picker-' + meta.id_file_picker).length) {
					return;
				}
				$image = $elm.find('img').css('max-height', '22px');
				$selected = $('<div class="input-group mb-2 me-2 flex-nowrap" style="width:auto"><span class="input-group-text rounded">' + $image[0].outerHTML + '<span class="ms-3">' + meta.nama_file + '</span></span><input type="hidden" id="id-file-picker-' + meta.id_file_picker + '" name="id_file_picker[]" value="' + meta.id_file_picker + '"/><button type="button" class="btn btn-danger btn-xs btn-del-bukti"><i class="fas fa-times"></i></button></div>');
				$('#list-file-bukti').prepend($selected);
