...
jQuery(document).ready(function () {
		...
		$bootbox = bootbox.confirm({
			...
			callback: function(confirmed) {
				if (confirmed) {
					...
					$.ajax({
						type: 'POST',
						url: base_url + 'pembayar/ajaxDeleteData',
						data: 'id=' + id,
						dataType: 'json',
	...
	function showForm(type='add', id = '') {
		...
		$.get(base_url + 'pembayar/ajaxGetFormData?id=' + id, function(html){
			$button.prop('disabled', false);
	...
	$('.btn-delete-all-data').click(function() {
		...
		$bootbox =  bootbox.dialog({
			...
			message: '<div class="px-2">' +
						'<p>Tindakan ini akan menghapus semua data pada database tabel pembayar</p>' +
				'</div>'+
			'</form>',
			buttons: {
				cancel: {
				...
				success: {
					...
					callback: function() 
					{
						...
						$.ajax({
							type: 'GET',
							url: base_url + 'pembayar/ajaxDeleteAllData',
							dataType: 'text',
							success: function (data) {
								data = $.parseJSON(data);
								console.log(data);
