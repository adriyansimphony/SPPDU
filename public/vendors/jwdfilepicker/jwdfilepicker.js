...
(function($) {
	...
	this.jwdfilepicker.init = function (options = {})
	{
		...
		$('body').undelegate('.btn-delete-file', 'click').delegate('.btn-delete-file', 'click', function() 
		{
			var $this = $(this),
		...
		*/
		...
		jwdDropzone.on("addedfile", function(file) 
		{
			...
			if(mime != 'image/png' && mime != 'image/jpg' && mime != 'image/jpeg' && mime != 'image/bmp') {
			  $(file.previewElement).find('img').attr('src', options.server_url + 'ajaxFileIcon?mime=' + mime + '&ext=' + ext);
			}
		});
